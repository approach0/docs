...
```
(in this example, when `indexer` service is not there, be careful to check modification time of `mnt` directory should match that of `nohup.out`, because `mnt` is created when index image producer has the lock)
